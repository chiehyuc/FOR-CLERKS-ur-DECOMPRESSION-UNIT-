...
if(r==="stand"){let sec=0;let t=setInterval(()=>{if(!document.body.contains($("#ward"))){clearInterval(t);return}sec++;$("#standTimer").textContent=sec;if(sec===8){document.querySelector(".a").style.left="37%";$("#out").textContent="TEAM MOVING. Your safe zone has expired."}if(sec===14){document.querySelector(".r").style.right="40%";$("#out").textContent="The herd is migrating."}},1000);document.querySelectorAll(".pos").forEach(x=>x.onclick=()=>{let p=x.dataset.p,msg={foot:"❌ Too visible. You may be asked something.",attending:"❌ Ambitious positioning. Dangerous.",computer:"❌ You are obstructing a workflow you do not understand.",corner:"❌ Suspicious. You now look lost.",hall:"✓ Excellent situational awareness."}[p];let y=$("#you");y.style.left=x.offsetLeft+"px";y.style.top=x.offsetTop+"px";$("#out").textContent=msg})}
 ...
 if(r==="note"){
  // Every button is purely additive: the base note is already grammatical,
  // and clicking a phrase only inserts that phrase into its fixed home.
  const extras=[
    {label:"overnight", slot:"sleep", text:" overnight"},
    {label:"without interruption", slot:"sleep", text:" without interruption"},
    {label:"per nursing report", slot:"sleep", text:", per nursing report"},
    {label:"otherwise", slot:"complaintPrefix", text:"Otherwise, "},
    {label:"at this time", slot:"complaint", text:" at this time"},
    {label:"overnight acute events", slot:"event", text:"No acute events were noted overnight."},
    {label:"clinically", slot:"statusPre", text:" clinically"},
    {label:"currently", slot:"statusPre", text:" currently"},
    {label:"without significant interval change", slot:"statusPost", text:" without significant interval change"},
    {label:"on current assessment", slot:"statusPost", text:" on current assessment"}
  ];
  let chosen=[];

  function picked(label){return chosen.includes(label)}
  function renderNote(){
    let sleep="Patient slept well";
    extras.filter(x=>x.slot==="sleep"&&picked(x.label)).forEach(x=>sleep+=x.text);
    sleep+=".";

    let event=extras.filter(x=>x.slot==="event"&&picked(x.label)).map(x=>x.text).join(" ");

    let complaint=(picked("otherwise")?"Otherwise, ":"")+"patient denies new complaints";
    extras.filter(x=>x.slot==="complaint"&&picked(x.label)).forEach(x=>complaint+=x.text);
    complaint=complaint.charAt(0).toUpperCase()+complaint.slice(1)+".";

    let status="Patient remains";
    // Fixed order keeps even deliberately redundant chart-speak grammatical.
    if(picked("currently")) status+=" currently";
    if(picked("clinically")) status+=" clinically";
    status+=" stable";
    if(picked("without significant interval change")) status+=" without significant interval change";
    if(picked("on current assessment")) status+=" on current assessment";
    status+=".";
  ...
  extras.forEach(item=>{
    let b=document.createElement("button");b.className="word";b.textContent=item.label;
    b.onclick=()=>{
      b.classList.toggle("selected");
      chosen.includes(item.label)?chosen.splice(chosen.indexOf(item.label),1):chosen.push(item.label);
      renderNote();
    };
    $("#words").appendChild(b)
  });
