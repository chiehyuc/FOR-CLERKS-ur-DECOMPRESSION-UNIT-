...
note:()=>shell("DOCUMENTATION SERVICES","Patient slept well. No new complaints. Stable. Your task: add words until it sounds medically expensive.",`<div class="note-box" id="noteText">Patient slept well. No complaints were reported. Patient remains stable.</div><div class="words" id="words"></div><button id="submitNote" class="action">SIGN NOTE</button><div id="out" class="screen">CHARACTER COUNT INFLATION: 0%</div>`),
...
if(r==="stand"){let sec=0;let t=setInterval(()=>{if(!document.body.contains($("#ward"))){clearInterval(t);return}sec++;$("#standTimer").textContent=sec;if(sec===8){document.querySelector(".a").style.left="37%";$("#out").textContent="TEAM MOVING. Your safe zone has expired."}if(sec===14){document.querySelector(".r").style.right="40%";$("#out").textContent="The herd is migrating."}},1000);document.querySelectorAll(".pos").forEach(x=>x.onclick=()=>{let p=x.dataset.p,msg={foot:"❌ Too visible. You may be asked something.",attending:"❌ Ambitious positioning. Dangerous.",computer:"❌ You are obstructing a workflow you do not understand.",corner:"❌ Suspicious. You now look lost.",hall:"✓ Excellent situational awareness."}[p];let y=$("#you");y.style.left=x.offsetLeft+"px";y.style.top=x.offsetTop+"px";$("#out").textContent=msg})}
 ...
 if(r==="note"){
  // Purely additive documentation inflation: every phrase has one fixed,
  // grammatical position. The underlying clinical information never changes.
  const extras=[
    {label:"reportedly", slot:"sleepPre"},
    {label:"overnight", slot:"sleepPost"},
    {label:"without interruption", slot:"sleepPost"},
    {label:"per nursing report", slot:"sleepReport"},
    {label:"no acute events", slot:"event"},
    {label:"throughout the night", slot:"eventTime"},
    {label:"new or additional", slot:"complaintModifier"},
    {label:"at this time", slot:"complaintTime"},
    {label:"clinically", slot:"statusPre"},
    {label:"without significant interval change", slot:"statusPost"},
    {label:"from prior assessment", slot:"statusCompare"}
  ];
  const chosen=new Set();
  const has=label=>chosen.has(label);

  function renderNote(){
    let sleep="Patient";
    if(has("reportedly")) sleep+=" reportedly";
    sleep+=" slept well";
    if(has("overnight")) sleep+=" overnight";
    if(has("without interruption")) sleep+=" without interruption";
    if(has("per nursing report")) sleep+=", per nursing report";
    sleep+=".";

    let event="";
    if(has("no acute events")){
      event="No acute events were noted";
      if(has("throughout the night")) event+=" throughout the night";
      event+=".";
    }

    let complaint="No ";
    if(has("new or additional")) complaint+="new or additional ";
    complaint+="complaints were reported";
    if(has("at this time")) complaint+=" at this time";
    complaint+=".";

    let status="Patient remains";
    if(has("clinically")) status+=" clinically";
    status+=" stable";
    if(has("without significant interval change")){
      status+=" without significant interval change";
      if(has("from prior assessment")) status+=" from prior assessment";
    } else if(has("from prior assessment")){
      status+=" compared with the prior assessment";
    }
    status+=".";

    $("#noteText").textContent=[sleep,event,complaint,status].filter(Boolean).join(" ");
    const base="Patient slept well. No complaints were reported. Patient remains stable.".length;
    const now=$("#noteText").textContent.length;
    $("#out").textContent=`CHARACTER COUNT INFLATION: ${Math.round((now-base)/base*100)}%`;
  }
  ...
  extras.forEach(item=>{
    ...
    b.onclick=()=>{
      if(chosen.has(item.label)) chosen.delete(item.label);
      else chosen.add(item.label);
      b.classList.toggle("selected",chosen.has(item.label));
      renderNote();
    };
    $("#words").appendChild(b);
  });
  ...
  renderNote();
  $("#submitNote").onclick=()=>$("#out").textContent="MAGNIFICENT. Additional words added. New information: negligible.";
}
