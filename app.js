...
if(r==="stand"){let sec=0;let t=setInterval(()=>{if(!document.body.contains($("#ward"))){clearInterval(t);return}sec++;$("#standTimer").textContent=sec;if(sec===8){document.querySelector(".a").style.left="37%";$("#out").textContent="TEAM MOVING. Your safe zone has expired."}if(sec===14){document.querySelector(".r").style.right="40%";$("#out").textContent="The herd is migrating."}},1000);document.querySelectorAll(".pos").forEach(x=>x.onclick=()=>{let p=x.dataset.p,msg={foot:"❌ Too visible. You may be asked something.",attending:"❌ Ambitious positioning. Dangerous.",computer:"❌ You are obstructing a workflow you do not understand.",corner:"❌ Suspicious. You now look lost.",hall:"✓ Excellent situational awareness."}[p];let y=$("#you");y.style.left=x.offsetLeft+"px";y.style.top=x.offsetTop+"px";$("#out").textContent=msg})}
 ...
 if(r==="note"){
  const additions=[
    {label:"overnight",apply:t=>t.replace("Patient slept well.","Patient slept well overnight.")},
    {label:"any",apply:t=>t.replace("No new complaints.","No new complaints of any kind.")},
    {label:"acute events",apply:t=>t.replace("Patient slept well overnight.","Patient slept well overnight. No acute events were noted.").replace("Patient slept well.","Patient slept well. No acute events were noted.")},
    {label:"currently",apply:t=>t.replace("Stable.","Currently stable.")},
    {label:"clinically",apply:t=>t.replace(/(?:Currently )?stable\./,"Clinically stable.")},
    {label:"remains",apply:t=>t.replace(/(?:Currently |Clinically )?stable\./,"Remains clinically stable.")},
    {label:"at this time",apply:t=>t.replace(/(?:Remains clinically|Clinically|Currently) stable\./,"Remains clinically stable at this time.")},
    {label:"without interval change",apply:t=>t.replace(/(?:Remains clinically stable at this time|Remains clinically stable|Clinically stable|Currently stable|Stable)\./,"Remains clinically stable without significant interval change.")},
    {label:"otherwise",apply:t=>t.replace("No new complaints of any kind.","Otherwise, no new complaints.").replace("No new complaints.","Otherwise, no new complaints.")}
  ];
  let chosen=[];
  const renderNote=()=>{
    let t="Patient slept well. No new complaints. Stable.";
    additions.forEach(a=>{if(chosen.includes(a.label))t=a.apply(t)});
    $("#noteText").textContent=t;
    $("#out").textContent=`CHARACTER COUNT INFLATION: ${chosen.length*17}%`;
  };
  additions.forEach(a=>{let b=document.createElement("button");b.className="word";b.textContent=a.label;b.onclick=()=>{b.classList.toggle("selected");chosen.includes(a.label)?chosen.splice(chosen.indexOf(a.label),1):chosen.push(a.label);renderNote()};$("#words").appendChild(b)});
